...
public class Bartok : MonoBehaviour
{
	...
	//This field is set to static to enforce that there is only 1 current player
	...
	void Awake()
	{
		...
		//Find the TurnLight by name if it's not hooked up yet
		if(turnLight == null)
		{
	...
	void LayoutGame()
	{
		...
		{
			GameObject tGO = new GameObject("_LayoutAnchor"); //Creates an empty GameObject named _LayoutAnchor in the Hierarchy
	...
	//This callback is used by the last card to be dealt at the beginning. It is only used once per game.
	...
	{
		//Pick the player to the elft of the human to go first. Note: players[0] is human
		PassTurn(1);
	}
	...
	public void PassTurn (int num = -1)
	{
		...
		if(CURRENT_PLAYER != null)
		{
			...
			//Check for Game Over and the need to reshuffle discards
			if(CheckGameOver())
			{
				return;
			}
	...
	//This Update method is used to test adding cards to player's hands
//	void Update()
//	{
