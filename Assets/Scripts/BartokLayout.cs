...
public class BartokLayout : MonoBehaviour {
	...
	public void ReadLayout (string xmlText)
	{
		...
		for (int i = 0; i < slotsX.Count; i++)
		{
			...
			//The layers are used to make sure that the correct cards are on top fo the others.
			//In Unity2D, all of our assets are effectively as the same Z depth, 
			//so sorting layers are used to differentiate between them
			...
			case "discardpile":
				...
				break;
			case "target":
				//The target card has a different layer from the discardPile
				target = tSD;
				break;
			case "hand":
